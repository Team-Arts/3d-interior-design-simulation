...
ModelManager::ModelManager() : device(nullptr)
{
    ...
    stateManager = std::make_unique<InteriorStateManager>();
}
...
bool ModelManager::LoadObjModelThreadFunction(const std::string &path, ID3D11Device *device, std::shared_ptr<Model> model, std::shared_ptr<LoadingProgress> progress)
{
    try
    {
        // 실제 로딩 진행
...
bool ModelManager::LoadGlbModelThreadFunction(const std::string &path, ID3D11Device *device, std::shared_ptr<GltfLoader> model, std::shared_ptr<LoadingProgress> progress)
{
    try
    {
        // 실제 로딩 진행
...
void ModelManager::RenderObjMaterialProperties(std::shared_ptr<Model> model)
{
    ...
    if (it != materials.end())
    {
        ...
        // 광택도 편집
        if (ImGui::SliderFloat("Gloss", &material.Shininess, 1.0f, 128.0f))
        {
            // 광택도가 변경됨
        }
...
void ModelManager::RenderImGuiWindow(HWND hwnd, ID3D11Device *device, float deltaTime)
{
    ...
    // 저장 버튼
    ImGui::SameLine();
    if (EnhancedUI::IconButton("저장", "S", ImVec2(100, 40)))
    {
        // 파일 저장 다이얼로그
        std::string savePath;
        if (OpenSaveFileDialog(hwnd, savePath))
        {
            if (stateManager->SaveStateToFile(savePath, this, roomModel, &camera, lightManager.get()))
            {
                // 저장 성공 메시지
                OutputDebugStringA("인테리어 상태가 저장되었습니다.\n");
            }
            else
            {
                // 저장 실패 메시지
                OutputDebugStringA("인테리어 상태 저장에 실패했습니다.\n");
            }
        }
    }
    ...
    // 로드 버튼
    ImGui::SameLine();
    if (EnhancedUI::IconButton("로드", "L", ImVec2(100, 40)))
    {
        // 파일 로드 다이얼로그
        std::string loadPath;
        if (OpenLoadFileDialog(hwnd, loadPath))
        {
            if (stateManager->LoadStateFromFile(loadPath, this, roomModel, &camera, lightManager.get(), device))
            {
                // 로드 성공 메시지
                OutputDebugStringA("인테리어 상태가 로드되었습니다.\n");
            }
            else
            {
                // 로드 실패 메시지
                OutputDebugStringA("인테리어 상태 로드에 실패했습니다.\n");
            }
        }
    }
...
void ModelManager::RenderToolbar(HWND hwnd)
{
    ...
    // 모델 제거 버튼
    if (EnhancedUI::IconButton("오브젝트 제거", EnhancedUI::ICON_REMOVE, buttonSize))
    {
...
void ModelManager::RenderPropertiesPanel()
{
    if (selectedModelIndex < 0 || selectedModelIndex >= models.size())
    {
        ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "왼쪽 모델 목록에서 모델을 선택하세요");
        return;
    }
...
void ModelManager::RenderObjMaterialPropertiesEnhanced(std::shared_ptr<Model> model)
{
    ...
    if (it != materials.end())
    {
        ...
        // 광택도 편집
        EnhancedUI::SliderFloat("광택도", &material.Shininess, 1.0f, 128.0f, "하이라이트의 집중도. 광택도가 높을 수록 더 날카롭게 빛난다.");

        EnhancedUI::RenderHeader("Texture");
        ...
        if (!material.DiffuseMapPath.empty())
        {
            ImGui::Text("현재 텍스처:");
            ImGui::SameLine();
            ImGui::TextColored(ImVec4(0.4f, 0.8f, 0.4f, 1.0f), "%s", material.DiffuseMapPath.substr(material.DiffuseMapPath.find_last_of("/\\") + 1).c_str());
            ...
            // 텍스처 변경 버튼
            if (EnhancedUI::IconButton("Change Texture", EnhancedUI::ICON_IMPORT))
            {
                std::string texturePath;
                if (OpenTextureFileDialog(GetActiveWindow(), texturePath))
                {
...
void ModelManager::RenderGlbMaterialPropertiesEnhanced(std::shared_ptr<GltfLoader> model)
{
    ...
    if (it != materials.end())
    {
        ...
        // 메탈릭 인자 편집
        EnhancedUI::SliderFloat("Metallic Factor", &material.MetallicFactor, 0.0f, 1.0f, "0: Non-metal(plastic, ...), 1: Metal. It determines degree of reflection");

        // 러프니스 인자 편집
        EnhancedUI::SliderFloat("Roughness Factor", &material.RoughnessFactor, 0.0f, 1.0f, "0: Smooth(Mirror), 1: Rough(diffused). It determines fine roughness of surface.");

        // 텍스처 정보 표시
        EnhancedUI::RenderHeader("Texture Map");
        ...
        {
            ImGui::SameLine();
            ImGui::Text("%s", material.BaseColorTexturePath.substr(material.BaseColorTexturePath.find_last_of("/\\") + 1).c_str());
        }
        else
        {
            ImGui::SameLine();
        ...
        {
            ImGui::SameLine();
            ImGui::Text("%s", material.MetallicRoughnessTexturePath.substr(material.MetallicRoughnessTexturePath.find_last_of("/\\") + 1).c_str());
        }
        else
        {
            ImGui::SameLine();
        ...
        {
            ImGui::SameLine();
            ImGui::Text("%s", material.NormalTexturePath.substr(material.NormalTexturePath.find_last_of("/\\") + 1).c_str());
        }
        else
        {
            ImGui::SameLine();
        ...
        {
            ImGui::SameLine();
            ImGui::Text("%s", material.EmissiveTexturePath.substr(material.EmissiveTexturePath.find_last_of("/\\") + 1).c_str());
        }
        else
        {
            ImGui::SameLine();
        ...
        {
            ImGui::SameLine();
            ImGui::Text("%s", material.OcclusionTexturePath.substr(material.OcclusionTexturePath.find_last_of("/\\") + 1).c_str());
        }
        else
        {
            ImGui::SameLine();
...
void ModelManager::RenderModelProperties(int modelIndex)
{
    ...
    ImGui::Spacing();
    EnhancedUI::RenderHeader("Transform");
    ...
    else if (modelInfo.type == MODEL_GLB)
    {
        ...
        {
            ImGui::PushItemWidth(-1);
            EnhancedUI::SliderFloat("X##Pos", &glbModelInfo.Position.x, -30.0f, 30.0f);
            EnhancedUI::SliderFloat("Y##Pos", &glbModelInfo.Position.y, -30.0f, 30.0f);
            EnhancedUI::SliderFloat("Z##Pos", &glbModelInfo.Position.z, -30.0f, 30.0f);
            ImGui::PopItemWidth();
            ImGui::TreePop();
        }
        ...
        if (!animations.empty())
        {
            EnhancedUI::RenderHeader("Animation");
            ...
            if (ImGui::BeginCombo("Animation",
                                  (currentAnim >= 0 && currentAnim < animNames.size()) ? animNames[currentAnim].c_str() : "없음"))
            {
...
void ModelManager::RenderRoomProperties()
{
    ...
    colorChanged |= EnhancedUI::ColorEdit("창문 색상", (float *)&windowColor, "창문 색상 및 투명도");
...
void ModelManager::RenderStatusBar()
{
    ...
    // 좌측에 모델 수 표시
    ImGui::Text("Model: %d", (int)models.size());
...
int ModelManager::PickModel(const Ray &ray)
{
    ...
    for (int i = 0; i < models.size(); i++)
    {
        ...
        if (RayIntersectsBoundingBox(ray, box))
        {
            ...
            XMVECTOR distVec = XMVectorSubtract(center, rayOrigin);
